...
def resolve_hand_sign_text(hand_sign_id, labels, is_right_hand):
    return get_label_text(hand_sign_id, labels)
...
def looks_like_partially_open_hand(landmark_list):
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                # Keep post-processing minimal to avoid suppressing valid classes.
                ...
                # Keep motion history active only in motion mode.
                motion_anchor_name = 'none'
                if mode == 2:
                    use_wrist_anchor = closed_fist_stable
                ...
                if mode == 2:
                    ...
                    non_zero_recent_ids = [gesture_id for gesture_id in list(current_gesture_history)[-6:] if gesture_id != 0]
                    if non_zero_recent_ids:
                        smoothed_finger_gesture_id = Counter(non_zero_recent_ids).most_common(1)[0][0]
                        finger_gesture_text = str(smoothed_finger_gesture_id)

                if show_debug_details:
                    fist_hold = right_closed_fist_hold if is_right_hand else left_closed_fist_hold
                    debug_sign_text = f'{debug_sign_text} mraw:{finger_gesture_id} ma:{motion_anchor_name} cfh:{fist_hold}'
        ...
        if mode == 2 and not fast_visuals:
